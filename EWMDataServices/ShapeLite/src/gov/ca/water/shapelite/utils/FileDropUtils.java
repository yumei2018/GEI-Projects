...
public class FileDropUtils {
   ...
   */
  private static final Logger LOGGER = Logger.getLogger(FileDropUtils.class.getName());
   ...
   */
  private FileDropUtils() {

  }

  /**
   * Constructs a {@link FileDrop} with a default light-blue border and, if c is
   * a {@link java.awt.Container}, recursively sets all elements contained
   * within as drop targets, though only the top level container will change
   * borders.
   *
  ...
  public static void addListener(
    ...
    addListener(c, // Drop target
        BorderFactory.createMatteBorder(2, 2, 2, 2, FileDrop.DEFAULT_BORDER_COLOR),
        true, // Recursive
        listener);
  }   // end constructor
  ...
  public static void addListener(
    ...
    FileDrop drop = new FileDrop(c, dragBorder, recursive, listener);
  }   // end constructor

  /**
   * Supports Drag and Drop.
   *
   * @return A boolean that is true if the environment supports drag and drop.
   */
   ...
   *
   * @param evt The DropTargetDragEvent.
   * @return Boolean, true if dragging is ok.
   */
  static boolean isDragOk(final DropTargetDragEvent evt) {
    boolean ok = false;
   ...
   * components contained within c if c is a {@link java.awt.Container}.
   *
   * @param c The component to unregister as a drop target
   * @return Boolean, true if the drag drop hooks were removed from the
   * component.
   * @since 1.0
   */
  public static boolean removeListener(java.awt.Component c) {
    return removeListener(null, c, true);
  }   // end remove
   ...
   * @since 1.0
   */
  public static boolean removeListener(PrintStream out, Component c, boolean recursive) {
    if (supportsDnD()) {
      ...
      if (recursive && (c instanceof java.awt.Container)) {
        ...
        for (Component comp : comps) {
          removeListener(out, comp, recursive);
        }
        return true;
      } else {
        return false;
   ...
   * public class MyClass implements FileDropUtils.Listener
   * ...
   * public void filesDropped( java.io.File[] files )
   * {
   * ...
  ...
  public static class Event extends java.util.EventObject {
     ...
     * the {@link FileDropUtils} that initiated the event.
     *
  ...
  public static class TransferableObject implements Transferable {
    ...
    public final static DataFlavor DATA_FLAVOR
        = new DataFlavor(FileDropUtils.TransferableObject.class, MIME_TYPE);
    ...
    /**
     * Creates a new {@link TransferableObject} that wraps data. Along with the
     * {@link #DATA_FLAVOR} associated with this class, this creates a custom
     * data flavor with a representation class determined from
     ...
     * is returned by fetcher. No custom data flavor is set other than the
    ...
    /**
     * Creates a new {@link TransferableObject} that will return the object that
     * is returned by fetcher. Along with the {@link #DATA_FLAVOR} associated
     * with this class, this creates a custom data flavor with a representation
     * class dataClass and the MIME type
     * <tt>application/x-net.iharder.dnd.TransferableObject</tt>.
     *
     * @see Fetcher
     ...
     * @since 1.1
     */
    public final DataFlavor getCustomDataFlavor() {
      return customFlavor;
    ...
    public final Object getTransferData(DataFlavor flavor)
      ...
      if (flavor.equals(DataFlavor.stringFlavor)) {
        if (fetcher == null) {
          ...
          return fetcher.getObject().toString();
        }
      }
    ...
    /**
     * Returns <tt>true</tt> if flavor is one of the supported flavors. Flavors
