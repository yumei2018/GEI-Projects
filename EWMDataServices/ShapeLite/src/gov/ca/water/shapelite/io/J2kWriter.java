 ...
 * */
public class J2kWriter extends ImgWriter {
     ...
     * @see DataBlk
     * */
    public J2kWriter(BlkImgDataSrc imgSrc, 
     ...
     * */
    @Override
    public void write(int ulx, int uly, int width, int height) throws IOException{
        int k,j,i,c;
        ...
        if(buf==null || buf.length<3*width) {
            buf = new byte[3*width];
        }
        ...
        for(i=0; i<height; i++) {
            ...
            for(c=0; c<3; c++) {
                ...
                if(fracbits==0) {
                    for(k=db.offset+width-1, j=3*width-1+c-2; j>=0; k--) {
                        tmp = db.data[k]+shift;
